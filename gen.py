...
def gen_image1(chains_with_weights):
    ...
    pos = nx.nx_agraph.graphviz_layout(G, args="-Goverlap=compress -Nshape=box -LU0.05")
    ...
    nx.draw(G, pos, alpha = 1, edge_color='black', arrows=True, arrowsize=24)

    labels = {node: f"{node[:50]} ({node_weights[node]})" for node in G.nodes()}
    nx.draw_networkx_labels(G, pos, labels=labels, font_size=24)  # 调整字体大小以避免重叠
